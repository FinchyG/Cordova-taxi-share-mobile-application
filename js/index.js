...
function TaxiShare() {
    ...
    function addMarkerToMap(address) {
        ...
        // Test that an address has been passed to function 
        if (address) {
            ...
            var onSuccess = function (data) {
                ...
                // Create a coordinate point from data returned by nominatim 
                // for given address
                var point = {
                   lng: data[0].lon,
                   lat: data[0].lat
            ...
            navigator.nominatim.get(address, onSuccess);
        }
    }
    ...
    function updateMapAddresses() {
        ...
        function onSuccess(data) {
            ...
            // Test that object data has been passed to onSuccess
            console.log("data received: ", data);
            
            // Loop through matches and add a marker to the map for each
            // one's pickup location'
            for(let i = 0; i < data.data.length; i++) {
                addMarkerToMap(data.data[i].hire_address);
            }
        }
        
        // Construct url to get matches
        var url = BASE_URL + "matches";
        
        // Test that the url has been constructed correctly, then send the get to 
        // return all offer / request matches
        console.log("matches: sending GET to " + url);       
        $.ajax(url, { type: "GET", data: { userid: oucu},success: onSuccess });
    ...
    function offer(oucu, address, startTime, endTime) {
        ...
        function onSuccess(obj) {
            ...
            // Inform the user what happened
            if (obj.status == "success") {
                alert("Offer has been successfully placed.");
            } else if (obj.message) {
                alert(obj.message);
            } else {
                alert("Offer not placed");
            }
        }        
        
        // Construct url to send offer request to
        var url = BASE_URL + "orders";
        
        // Test that url has been constructed correclty, then send offer request
        console.log("offer: sending POST to " + url);       
        $.ajax(url, { type: "POST", data: { userid: oucu, type: 0, address: address, 
            start: startTime, end: endTime }, success: onSuccess });
    ...
    function request(oucu, address, startTime) {
        ...
        function onSuccess(obj) {
            ...
            if (obj.status == "success") {
                ...
                alert("request not placed");
            }
        }        
        
        // Construct url to send request request to
        var url = BASE_URL + "orders";
        
        // Test that url has been constructed correclty, then send request request
        console.log("request: sending POST to " + url);       
        $.ajax(url, { type: "POST", data: { userid: oucu, type: 1, address: address, 
            start: startTime }, success: onSuccess });        
    ...
    function useCamera() {
        ...
        function onSuccess(data) {
            console.log("camera worked", data);
        }
    ...
    // Controller function to reset the location map to the location of this device
    ...
    this.updateMap = function () {
        // Note that you will need to make changes in: 
            // UpdateMapAddresses FR2.1 and addMarkerToMap FR2.2
            // But no changes are required in this function.
        // Update map now
        updateMapAddresses();
    };
    ...
    this.registerUser = function () {
        ...
        var oucu = getInputValue("newOUCU", "user1");
    ...
    this.offerTaxi = function () {
        ...
        // TODO adjust the following to get the required data from your HTML view
        var oucu = getInputValue("OUCU", "user1");
        var address = getInputValue("pickupAddress", "Milton Keynes Central Station");
        var startTime = getInputValue("startTime", defaultStartTime); // eg. 2020:12:18:14:38
        var hours = getInputValue("waitingTime", "1"); // duration in hours        
    ...
    this.requestTaxi = function () {
        // TODO adjust the following to get the required data from your HTML view
        var oucu = getInputValue("OUCU", "user1");
        var address = getInputValue("pickupAddress", "Open University, Milton Keynes");
        var startTime = getInputValue("startTime", convertToOrderTime(new Date()));
    ...
    // Controller function for user to take a photo
